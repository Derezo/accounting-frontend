...
import { QuoteFilters as IQuoteFilters, QuoteStatus } from '@/types/api'
...
export function QuoteFilters({ filters, onFiltersChange }: QuoteFiltersProps) {
  const [localFilters, setLocalFilters] = useState<IQuoteFilters>(filters)
  const { data: customersData } = useCustomers({ limit: 100 })

  const handleFilterChange = (key: keyof IQuoteFilters, value: any) => {
  ...
  const clearFilters = () => {
    const emptyFilters: IQuoteFilters = {}
    setLocalFilters(emptyFilters)
    onFiltersChange(emptyFilters)
  }

  const hasActiveFilters = Object.keys(filters).some(key => {
    const value = filters[key as keyof IQuoteFilters]
